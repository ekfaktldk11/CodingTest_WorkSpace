...
# 왼쪽으로 회전
def turn_left():
    # 함수 외부에 있는 변수를 함수내에서 전역 변수처럼 사용하려면 main을 제외한 모든 함수에 해당 변수에 대한 global 처리가 필수
    global direction
    direction -= 1
    if direction == -1:
        direction = 3
...
- !!! 내 풀이는 최초위치에 대한 방문 회수는 반영했지만 방문 했는지 안했는지에 대한 표시를 빼먹음 !!!
"""
...
- 최초 위치에 대한 방문횟수, 방문유무 를 고려하는 것을 빼먹지말자
- main 함수를 제외한 scope 에서 변수를 전역변수 처럼 사용하게 하기 위한 변수의 global 처리
"""
